...
namespace LoadImpact_Script_Export
{
    ...
    [ProfferFormat("LI lua-script", "Load Impact Lua-script")]
    public class LI_Export : ISessionExporter
    {
        ...
        private static string k6Head = @"// This script is generated by Fiddler k6 exporter
...
";
        ...
        private static string luaHead = @"-- This script is generated by Fiddler Lua exporter
...
";
        ...
        private void addLuaMultipart(MimeEntity mimeBody,
                                     ref int iSequence,
                                     ...
                                     ref StringBuilder strbBody)
        {
            ...
            foreach (MimePart part in multipart)
            {
                ...
                {
                    addLuaMultipart(part, ref iSequence, ref strbHead, ref strbBody);
                }
                else
                {
                    ...
                    if ((part.ContentTransferEncoding == ContentEncoding.Binary) || (Regex.IsMatch(strBody, "[\x00-\x08\x0E-\x1F\x80-\xFF]")))
                    {
                        ...
                        string binBase64 = Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(strBody), Base64FormattingOptions.None);
        ...
        private void terminateLuaBatches(Dictionary<string, List<Session>> dictBatches,
                                         ref StringBuilder strbBody,
                                         ref StringBuilder strbHead,
                                         ref int iBinarySequence,
                                         string sFilename)
        {
            foreach (KeyValuePair<string, List<Session>> kvp in dictBatches)
            {
                ...
                strbBody.AppendLine("\thttp.request_batch({");
                ...
                foreach (Session oS in kvp.Value)
                {
                    ...
                    strRequest = "\t\t{\"" + oS.RequestMethod.ToUpper() + "\", \"" + oS.fullUrl + "\", auto_decompress = true";
        ...
        #region k6Export
        private bool k6Export(string sFilename, 
                              Session[] oSessions,
...
//            List<Session> liGroup = new List<Session>();
            ...
            else if (sFormat == "LI lua-script")
            {
                if (string.IsNullOrEmpty(sFilename)) sFilename = Fiddler.Utilities.ObtainSaveFilename("Export As " + sFormat, "LI Lua Files (*.lua)|*.lua");
            }
            else
            {
            ...
            try
            {
                if (sFormat == "k6 javascript")
                {
                    bResult = k6Export(sFilename, oSessions, evtProgressNotifications);
                }
                else if (sFormat == "LI lua-script")
                {
                    bResult = luaExport(sFilename, oSessions, evtProgressNotifications);
                }
                else
                {
                    // No, this should not even be possible!
                    MessageBox.Show("Unsupported export type: " + sFormat);
                    return false;
                }
            }
            catch (Exception eX)
